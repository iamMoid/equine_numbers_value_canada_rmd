...
There is an interesting paper about the weights of horses [@milner1969weight].
...
```{r plot horses, fig.width=10, fig.height=3, fig.cap="Figure 1. Historical number of horses for all provinces in Canada", out.width = "100%"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable(max_horses, caption = "Table 1. Maximum and Minimum count of horses by Province")
```

Below we zoom in and look at the province of Alberta:

```{r plot province, fig.width=5, fig.height=4, fig.cap="Figure 2. Historical number of horses in Alberta", out.width="50%"}
province_plot <- horse_pop %>% 
  ...
  xlab("Year") +
  ylab("Number of horses") +
  ggtitle("Historical number of horses in Alberta") +
  facet_grid(~GEO) +
  scale_y_continuous(labels = scales::comma) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
province_plot
